...
def V12(r, M, rQ, l):
    epsilon = 1
    return nu_12(l)*G(r, M, rQ)/H(r) + epsilon*np.sqrt(nu_12(l))*F(r, M, rQ)*(dF_dr(r, M, rQ)/(2*r*np.sqrt(F(r, M, rQ))) - np.sqrt(F(r, M, rQ))/r**2)
...
file1 = open("/home/sarthaka/Downloads/blackhawk_v2.3/scripts/greybody_scripts/greybody_factors/charged/fM_charged_0.5_high_py.txt", "w")
        ...
        for i in range(nbmodes):
            ...
            for idx in range(deg_l):
                ...
                if m>-l[i]:
                    ...
                    s0[k][j][i][idx] = solver(M, rQ[k], omega[j], l[i])
