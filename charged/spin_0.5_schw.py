...
def rstar(r, M):
    return r + rplus(M)**2/(rplus(M))*np.log(r/rplus(M) - 1) 
...
def V12(r, M, l):
    epsilon = 1
    return nu_12(l)*G(r, M)/H(r) + epsilon*np.sqrt(nu_12(l))*F(r, M)*(dF_dr(r, M)/(2*r*np.sqrt(F(r, M))) - np.sqrt(F(r, M))/r**2)
...
def solver(M, omega, l):
    rp = rplus(M)
    rminf = rstar(rp + prox, M)
    rpinf = rstar(rp+far, M)
    r_init = rp + prox  
    phase = -omega * rminf
    ...
    def system(t, y):
        ...
        dr_drs = (r_curr - rp) * (r_curr) / (r_curr**2)
...
file1 = open("/home/sarthaka/Downloads/blackhawk_v2.3/scripts/greybody_scripts/greybody_factors/fM_schw_0.5_low_py.txt", "w")
## change the name low to high depending on the energy value


s0      = [[[[None for _ in range(int(2 * l[i] + 1))] for i in range(nbmodes)] for j in range(nbener)] for k in range(nbrQ)]
Ain0    = [[[[0j   for _ in range(int(2 * l[i] + 1))] for i in range(nbmodes)] for j in range(nbener)] for k in range(nbrQ)]
Aout0   = [[[[0j   for _ in range(int(2 * l[i] + 1))] for i in range(nbmodes)] for j in range(nbener)] for k in range(nbrQ)]
contrib = [[[[0.0  for _ in range(int(2 * l[i] + 1))] for i in range(nbmodes)] for j in range(nbener)] for k in range(nbrQ)]
...
def Temp(M):
    return (rplus(M))/(4*np.pi*rplus(M)**2)
        ...
        for i in range(nbmodes):
            ...
            if optimize and i>0 and Al0[k,j]>10**(-100):
                ...
                if abs(max_contrib1/Al0[k,j])<10**(-5):
                    break
            deg_l = int(2 * l[i] + 1)
            for idx in range(deg_l):
                m = -l[i] + idx
                if m>-l[i] and optimize and Al0[k,j]>10**(-100):
                    contrib2 = contrib[k][j][i][idx-1]
                    if abs(contrib2/Al0[k,j])<10**(-5):
                        break
                if m>-l[i]:
                    s0[k][j][i][idx] = s0[k][j][i][idx-1]
                else: 
                    s0[k][j][i][idx] = solver(M, omega[j], l[i])

                rpinf = rstar(rplus(M) + far, M)
                sol_obj = s0[k][j][i][idx]
                
                X_val  = sol_obj.y[1][-1] + 1j * sol_obj.y[2][-1]
                dX_val = sol_obj.y[3][-1] + 1j * sol_obj.y[4][-1]
                Ain0[k][j][i][idx] = (X_val + dX_val/(1j * omega[j])) / (2 * np.exp(1j * omega[j] * rpinf))
                Aout0[k][j][i][idx] = (X_val - dX_val/(1j * omega[j])) / (2 * np.exp(-1j * omega[j] * rpinf))
                ...
                term = (1.0 / abs(Aout0[k][j][i][idx])**2) * be_factor
                contrib[k][j][i][idx] = term
                
                Al0[k, j] = Al0[k, j] + contrib[k][j][i][idx]
