...
# alpha_rough = np.genfromtxt("/home/sarthaka/Downloads/blackhawk_v2.3/scripts/greybody_scripts/greybody_factors/charged/alpha_charged.txt")
alpha_rough = np.genfromtxt("/home/batman/Documents/blackhawk_v2.3/alpha_charged.txt")
...
def rplus(M, Q):
    q = abs(Q/M)
    q = np.where(M_pl*q>=0.9, 0.9/M_pl, q)
    return M*(1+np.sqrt(1-(M_pl*q)**2))/M_pl**2
...
def arg(M,Q):
    return np.pi*rplus(M,Q)**2*mD**2/(eD*abs(Q))
...
mass_initial = 9.6*10**(12)*g_to_GeV
real_q = 0.01*mass_initial/M_pl
...
def Temp(M, Q):
    # if Q >= M/M_pl: return 0.0
    kappa = (M_pl**2)*np.sqrt(1-(Q*M_pl/M)**2)/(M*(1+np.sqrt(1-(Q*M_pl/M)**2))**2)
    return kappa*10**(9)/(2 * np.pi)
...
ax.set_yscale("log")
ax.set_xlabel("$t [yr]$")
ax.set_ylabel(r"$M_{PBH}$ [g]", color='b')
ax.plot(t_yr, M_g, 'b-', label=r'RN+BSM ($M_{PBH}$)')
f.show()
...
ax.set_yscale("log")
ax.set_xlabel("$t [yr]$")
ax.set_ylabel(r"$-Log_{10}[1 - Q^*_D]$", color='r')
ax.plot(t_yr, mod_y, 'r-', label=r'RN+BSM ($M_{PBH}$)')
f.show()
...
ax.set_title(r'Evolution Trajectory for arg')
...
ax.set_yscale("log")
ax.set_xlabel("$t [yr]$")
ax.set_ylabel(r"$Q_D^*$", color='r')
ax.plot(t_yr, q_star_refined, 'r-', label=r'RN+BSM ($M_{PBH}$)')
f.show()

def dQ_stardt(M, Q):
    func = M_pl*(alpha_smooth(M, Q)*q_star_refined*M_pl**3/M**2-eD*q_star_refined**3*M**3/(2*mD**2*np.pi**3*rplus(M, Q)**3)*np.exp(-np.pi*rplus(M,Q)**2*mD**2*M_pl/(eD*q_star_refined*M))*(1-q_star_refined**2*M/(M_pl**2*rplus(M, Q))))/M
    return func
...
ax.set_yscale("log")
ax.set_xlabel("$t [yr]$")
ax.set_ylabel(r"$\frac{dQ_D^*}{dt}$", color='r')
ax.plot(t_yr, dQ_stardt(mass_evol, charge_evol), 'r-', label=r'RN+BSM ($M_{PBH}$)')
f.show()
