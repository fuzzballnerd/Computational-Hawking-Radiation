...
# ─────────────────────────────────────────────────────────────────────
alpha_rough = np.genfromtxt("/home/batman/Downloads/blackhawk_v2.3/scripts/greybody_scripts/greybody_factors/charged/new_alpha_charged.txt")
...
Qom0   = 0.99           # initial charge-to-mass ratio
...
sigma_max = 2.0 / 3.0              # safe upper bound (×2 of the Hawking evap value)
print(f"\nscale     = {scale:.3e}")
print(f"sigma_max = {sigma_max:.3f}  (pure Hawking σ_evap ≈ 1/3)")
print(f"t_max     ≈ {sigma_max * scale * Ms**3 / (hbar * c_SI):.3e} s")
...
sigma_arr = sol.t
mu_arr  = np.clip(sol.y[0], 0.0, None)
Y_arr   = np.clip(sol.y[1], 0.0, 1.0)

# ─────────────────────────────────────────────────────────────────────
#  Convert to physical units
# ─────────────────────────────────────────────────────────────────────
t_s  = sol.t * scale * Ms**3 / (hbar * c_SI)   
t_yr = t_s / yr_to_s
...
plt.tight_layout()
plt.savefig('bh_time_evolution.png', dpi=150, bbox_inches='tight')
plt.show()
print("Plot saved → bh_time_evolution.png")