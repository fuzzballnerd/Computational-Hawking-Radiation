...
def V0(r, M, rQ, l):
    return nu_0(l)*G(r, M, rQ)/H(r) + F(r, M, rQ)*dF_dr(r, M, rQ)/r 
...
file1 = open("/home/sarthaka/Downloads/blackhawk_v2.3/scripts/greybody_scripts/greybody_factors/charged/fM_charged_0_high_py.txt", "w")
    ...
    for j in range(nbener):
        ...
        for i in range(nbmodes):
            ...
            if optimize and i>0 and Al0[k,j]>10**(-100):
                ...
                if abs(max_contrib1/Al0[k,j])<10**(-5):     ### term neglected if contribution is less
                    break
            for m in range(-l[i], l[i]+1):
                if m>-l[i] and optimize and Al0[k,j]>10**(-100):
                    contrib2 = contrib[k][j][i][m+l[i]-1]
                    if abs(contrib2/Al0[k,j])<10**(-5):     ### term neglected if contribution is less
                        break
                if m>-l[i]:
                    s0[k][j][i][m+l[i]] = s0[k][j][i][m+l[i]-1]            ### uses symmetry for m in the absence of rotation
                else:                           
                    s0[k][j][i][m+l[i]] = solver(M, rQ[k], omega[j], l[i])

                rpinf = rstar(rplus(M, rQ[k]) + 350, M, rQ[k])
